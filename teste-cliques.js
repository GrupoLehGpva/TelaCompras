...
async function tela(b, {qs='?uid=U0BL5JPQX97', webhook=200, facilitadores=FAC}={}){
  ...
  await p.route('**supabase.co/**', r => {
    ...
    /* A busca do facilitador virou RPC, que também é POST — tem que vir antes
       do POST genérico, senão a rota do formulário engole a da função. */
    ...
    if(r.request().method() === 'POST')
      return r.fulfill({status:201,contentType:'application/json',body:JSON.stringify([{id:'novo-1'}])});
    if(u.includes('centros_custo')) return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify(CENTROS)});
